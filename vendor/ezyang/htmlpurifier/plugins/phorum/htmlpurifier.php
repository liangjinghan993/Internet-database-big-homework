...
function phorum_htmlpurifier_format($data)
{
    ...
    foreach($data as $message_id => $message){
        if(isset($message['body'])) {
            ...
            // databasemigration might edit this array, that's why it's defined
            // so early
...
/**
 * Indicate that data is fully HTML and not from databasemigration, invalidate
 * previous caches
...
function phorum_htmlpurifier_common()
{
    ...
    // load databasemigration
    if (file_exists(dirname(__FILE__) . '/migrate.php')) {
        include(dirname(__FILE__) . '/migrate.php');
    } else {
        echo '<strong>Error:</strong> No databasemigration path specified for HTML Purifier, please check
