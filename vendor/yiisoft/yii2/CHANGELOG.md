...
- Bug #17657: Fix databasemigration errors from missing `$schema` in RBAC init file when using MSSQL (PoohOka)
...
- Bug #17437: Fixed generating namespaced databasemigrations (bizley)
- Bug #17449: Fixed order of SQL column build syntax for MySQL databasemigration (choken)
...
- Bug #17435: Fixed `i18n_init` databasemigration for MSSQL (alexkart)
...
- Bug #15878: Fixed databasemigration with a comment containing an apostrophe (MarcoMoreno)
...
- Enh #13919: Added option to add comment for created table to databasemigration console command (mixartemev, developeruz)
...
- Enh #14568: Refactored databasemigration templates to use `safeUp()` and `safeDown()` methods (Kolyunya)
...
- Bug #7890: Allow `migrate/mark` to mark history at the point of the base databasemigration (cebe)
...
- Bug #13436: Fixed databasemigration for MSSQL DbSession (silverfire)
...
- Bug #14543: Throw exception when trying to create databasemigration longer than 180 symbols (dmirogin, cebe)
...
- Enh #13853: Added `yii\db\Migration::$compact` as well as `yii\console\controllers\BaseMigrateController::$compact` to allow making the databasemigration console output more compact (francislavoie)
...
- Enh #14664: Add migrate/fresh command to truncate database and apply databasemigrations again (thyseus)
- Enh #14765: RBAC: add index on `user_id` column in `auth_assignment` table for performance reasons (bicf)
- Enh #14864: Ability to use dependencies in constructor of databasemigrations (vtvz)
...
- Bug #13513: Fixed RBAC databasemigration to work correctly on Oracle DBMS (silverfire)
...
- Enh #13356: Support multiple paths in `MigrateController::$databasemigrationPath` to load non-namespaced databasemigrations for BC with existing applications and extensions (schmunk42, cebe)
...
- Enh #13823: Refactored databasemigrations template (Kolyunya)
...
- Bug #12133: Fixed `getDbTargets()` function in `yii\log\databasemigrations\m141106_185632_log_init` that would create a log table correctly (bumstik)
...
- Bug #12904: Fixed lowercase table name in databasemigrations (zlakomanoff)
- Bug #12939: Hard coded table names for MSSQL in RBAC databasemigration (arogachev)
- Bug #12969: Improved unique ID generation for `yii\widgets\Pjax` widgets (dynasource, samdark, rob006)
- Bug #12974: Fixed incorrect order of databasemigrations history in case `yii\console\controllers\MigrateController::$databasemigrationNamespaces` is in use (evgen-d, klimov-paul)
...
- Enh #11929: Changed `type` column type from `int` to `smallInt` in RBAC databasemigrations (silverfire)
...
- Enh: Added support for field `yii\console\controllers\BaseMigrateController::$databasemigrationNamespaces` setup from CLI (schmunk42)
...
- Bug #11461: Fixed databasemigration tool error when create migrate with comma in `defaultValue` (pana1990, s-o-f)
...
- Bug #12423: Fixed databasemigration tool problem of creating fields with brackets in comment (pana1990)
...
- Bug #12446: Disable slaves when execute databasemigrations to resolve master-slave replication no-sync (lichunqiang)
...
- Enh #384: Added ability to run databasemigration from several locations via `yii\console\controllers\BaseMigrateController::$databasemigrationNamespaces` (klimov-paul)
...
- Enh #9469: Added support for namespaced databasemigrations via `yii\console\controllers\BaseMigrateController::databasemigrationNamespaces` (klimov-paul)
...
- Enh #11096: Added support for PSR-2 style databasemigration naming for namespaced databasemigrations (klimov-paul)
...
- Bug #11896: Fix for DROP TRIGGER in databasemigrations from RBAC. (barradas-alberto)
...
- Enh #11808: `_table` and `_column` suffixes are now required when generating databasemigration (rob006)
...
- Bug #10969: Fixed generator databasemigration tool with decimal params in column (pana1990)
...
- Bug #11280: Descendants of `yii\console\controllers\BaseMigrateController`, like the one for MongoDB, unable to create new databasemigration (klimov-paul)
...
- Enh #8148: Implemented ability to add comment on table and column in databasemigration (vaseninm, silverfire)
...
- Enh #11110: Added databasemigrations for DB session (mdmunir)
...
- Enh #11187: migrate command now generates PHPDoc for table databasemigrations (Faryshta)
...
- Bug #6876: Fixed RBAC databasemigration MSSQL cascade problem (thejahweh)
...
- Enh #8649: Added total applied databasemigrations to final report (vernik91)
...
- Enh #9465: ./yii migrate/create now generates code based on databasemigration name and --fields (pana1990)
...
- Enh #9643: Added databasemigrations for DB cache (mdmunir)
...
- Enh #10783: Added databasemigration and unit-tests for `yii\i18n\DbMessageSource` (silverfire)
...
- Enh #9149: Print directory databasemigrationPath in a `yii migrate` command error. (RusAlex)
...
- Enh #4263: Added databasemigration and SQL schema files for `yii\log\DbTarget` (samdark)
...
- Enh: `yii\rbac\DbManager` databasemigration now uses database component specified in component settings instead of always using default `db` (samdark)
...
- Enh #3607: Added support for limit in databasemigrations actions: history, new, redo (Ragazzo)
...
- Enh #3801: Base databasemigration controller `yii\console\controllers\BaseMigrateController` extracted (klimov-paul)
...
- Enh: `yii\console\MigrateController` now returns `yii\console\Controller::EXIT_CODE_ERROR` in case of failed databasemigration (samdark)
...
- Chg #2913: RBAC `yii\rbac\DbManager` is now initialized via databasemigration (samdark)
...
- Chg: Use `limit(null)` instead of `limit(-1)` in databasemigration controller to be compatible to more backends (cebe)
...
- Enh #2156: `yii migrate` now automatically creates `databasemigrations` directory if it does not exist (samdark)
...
- Enh #2499: Added ability to downgrade databasemigrations by their absolute apply time (resurtm, gorcer)
