...
/**
 * Manages application databasemigrations.
 *
 * A databasemigration means a set of persistent changes to the application environment
 * that is shared among different developers. For example, in an application
 * backed by a database, a databasemigration may refer to a set of changes to
 * the database, such as creating a new table, adding a new table column.
 *
 * This command provides support for tracking the databasemigration history, upgrading
 * or downloading with databasemigrations, and creating new databasemigration skeletons.
 *
 * The databasemigration history is stored in a database table named
 * as [[databasemigrationTable]]. The table will be automatically created the first time
 ...
 *
 * ```
 * # creates a new databasemigration named 'create_user_table'
 * yii migrate/create create_user_table
 *
 * # applies ALL new databasemigrations
 * yii migrate
 *
 * # reverts the last applied databasemigration
 * yii migrate/down
 * ```
 *
 * Since 2.0.10 you can use namespaced databasemigrations. In order to enable this feature you should configure [[databasemigrationNamespaces]]
 ...
 *             'databasemigrationNamespaces' => [
 *                 'app\databasemigrations',
 *                 'some\extension\databasemigrations',
 *             ],
 *             //'databasemigrationPath' => null, // allows to disable not namespaced databasemigration completely
 *         ],
 *     ],
 * ];
 * ```
...
{
    /**
     * Maximum length of a databasemigration name.
     * @since 2.0.13
     */
    const MAX_NAME_LENGTH = 180;

    /**
     * @var string the name of the table for keeping applied databasemigration information.
     */
    public $databasemigrationTable = '{{%databasemigration}}';
    /**
     * {@inheritdoc}
     */
    public $templateFile = '@yii/views/databasemigration.php';
    /**
     * @var array a set of template paths for generating databasemigration code automatically.
     *
    ...
    /**
     * @var array column definition strings used for creating databasemigration code.
     *
     ...
     * when applying databasemigrations. Starting from version 2.0.3, this can also be a configuration array
    ...
    public function options($actionID)
    {
        return array_merge(
            ...
            ['databasemigrationTable', 'db'], // global for all actions
            $actionID === 'create'
    ...
    public function optionAliases()
    {
        return array_merge(parent::optionAliases(), [
            'C' => 'comment',
            'f' => 'fields',
            'p' => 'databasemigrationPath',
            't' => 'databasemigrationTable',
            'F' => 'templateFile',
            'P' => 'useTablePrefix',
            'c' => 'compact',
        ]);
     ...
     * @return \yii\db\Migration the databasemigration instance
     */
    ...
    {
        if ($this->db->schema->getTableSchema($this->databasemigrationTable, true) === null) {
            $this->createMigrationHistoryTable();
        }
        $query = (new Query())
            ->select(['version', 'apply_time'])
            ->from($this->databasemigrationTable)
            ->orderBy(['apply_time' => SORT_DESC, 'version' => SORT_DESC]);

        if (empty($this->databasemigrationNamespaces)) {
            $query->limit($limit);
    ...
    /**
     * Creates the databasemigration history table.
     */
    protected function createMigrationHistoryTable()
    {
        $tableName = $this->db->schema->getRawTableName($this->databasemigrationTable);
        $this->stdout("Creating databasemigration history table \"$tableName\"...", Console::FG_YELLOW);
        $this->db->createCommand()->createTable($this->databasemigrationTable, [
            'version' => 'varchar(' . static::MAX_NAME_LENGTH . ') NOT NULL PRIMARY KEY',
            'apply_time' => 'integer',
        ])->execute();
        $this->db->createCommand()->insert($this->databasemigrationTable, [
    ...
    protected function addMigrationHistory($version)
    {
        ...
        $command->insert($this->databasemigrationTable, [
            'version' => $version,
            'apply_time' => time(),
        ])->execute();
    }
    ...
    {
        $command = $this->db->createCommand();
        $command->delete($this->databasemigrationTable, [
            'version' => $version,
        ])->execute();
    }

    private $_databasemigrationNameLimit;
    ...
    {
        if ($this->_databasemigrationNameLimit !== null) {
            return $this->_databasemigrationNameLimit;
        }
        $tableSchema = $this->db->schema ? $this->db->schema->getTableSchema($this->databasemigrationTable, true) : null;
        if ($tableSchema !== null) {
            return $this->_databasemigrationNameLimit = $tableSchema->columns['version']->size;
        }
    ...
    /**
     * Parse the command line databasemigration fields.
